...
PREDICT_KEY = os.environ.get(
    'PREDICT_KEY',
    'AIzaSyAptd6M3SUokdQKMgCAssv-adCOP7fO8s8'
)
...
class Models:
    ...
    def fetch_product_id(self):
        ...
        self.client.close()
        return utils.convert_result_to_dataframe(result, columns, "records")
    ...
    def fetch_category(self):
        ...
        response = json.loads(
            json.loads(
                json.dumps(
                    utils.convert_result_to_dataframe(
                        result,
                        columns,
                        "records"
                    )
    ...
    def get_product_recommendation_custom(self):
        if self.requested_data.get('eventType') is not None or self.requested_data.get('eventType') is not '':
            placement_name = self.requested_data.get('placement_name')
            payload = json.dumps(self.requested_data.get('params'))
        else:
            placement_name = ""
            payload = ""
        url = """https://recommendationengine.googleapis.com/v1beta1/projects/%s/locations/global/catalogs/default_catalog/eventStores/default_event_store/placements/%s:predict?key=%s""".format(
            PROJECT_ID,
            placement_name,
            PREDICT_KEY
        )
